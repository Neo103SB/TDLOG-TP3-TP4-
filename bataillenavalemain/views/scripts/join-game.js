...
// url complète de l'api join-game
const url = host + "/join-game";

// Au chargement de la page, on ajoute un listener au formulaire join-game-form,
// à la soumission la fonction join_game(event) sera appelée
...
async function join_game(event) {
  ...
  // Appel de l'API join-game qui se fait en asynchrone
  // le await pour stocker le résultat dans response quand il sera reçu
  const response = await postData(url, formDataJsonString)
  // Récupérer l'element html identifié par "join-game-result"
  // afin de mettre le message de retour de l'api
  const resultJoinGame = document.getElementById("join-game-result");
  let h1 = document.createElement('H1');
  resultJoinGame.appendChild(h1)
  if (!response.ok) {
    ...
    h1.innerHTML = "Rejoindre la partie avec l'id : " + gameId + "<br>";
...
resultJoinGame.style.visibility = "visible";
...
const playerName = document.getElementById("player-name").value;
linkToVessels.href = "manage_fleet.html?game-id=" + gameId + "&player-name=" + playerName;
linkToVessels.text = "Rejoindre la flotte !"
h1.appendChild(linkToVessels);
h1.className = "success";
}
// on récupère l'objet représentant le formulaire et le masquer
